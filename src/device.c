...
/***********************************************************************
* Get handle and information of ps3 hdd/image.
* 
* ps3_context *ctx = ps3 device information
* const char *path = device path, e.g. /dev/sdb
***********************************************************************/
s32 get_device_handle(ps3_context *ctx, const char *path)
{
	s32 dev;
	...
	struct disklabel *pt_vf;
	
	// get device handle
	if((dev = open(path, O_RDWR)) <= 0)
		return -1;
	
	// get sector 0(first of ps3 main partition table)
	// and sector 8(first of VFLASH partition table, if a VFLASH)
  pread(dev, sec_0, SECTOR_SIZE, 0 * SECTOR_SIZE);
  _es16_buffer(sec_0, SECTOR_SIZE);
  pread(dev, sec_8, SECTOR_SIZE, 8 * SECTOR_SIZE);
  _es16_buffer(sec_8, SECTOR_SIZE);
  ...
  // is partition table magic?
    ...
    aes_xts_crypt(&xts_dec_vf, 8, SECTOR_SIZE, tmp, tmp);
    
    pt_vf = (struct disklabel *)tmp;
		
		// is partition table magic?
		if((_ES64(pt_vf->d_magic1) == MAGIC1) && (_ES64(pt_vf->d_magic2) == MAGIC2)) {
...
s64 block_read(ps3_context *ctx, u8 *buf, s64 n_sec, s64 sec_num)
{
	...
	n_read = pread(ctx->dev, buf, n_sec * SECTOR_SIZE, sec_num * SECTOR_SIZE);
...
s64 block_write(ps3_context *ctx, u8 *buf, s64 n_sec, s64 sec_num)
{
  ...
  switch(ctx->ps3_type) {
	...
	_es16_buffer(buf, n_sec * SECTOR_SIZE);
	n_write = pwrite(ctx->dev, buf, n_sec * SECTOR_SIZE, sec_num * SECTOR_SIZE);
...
s64 device_read(ps3_context *ctx, u8 *buf, u64 numbytes, s64 dev_off)
{
  ...
  block_read(ctx, tmp, 1, seq_nr);
  
	if(numbytes <= sec_rest) {
		memcpy(buf, tmp + byte_rest, numbytes);
		return numbytes;
	}
	
	if(numbytes > sec_rest) {
		memcpy(buf, tmp + byte_rest, sec_rest);
		...
		if((remain / SECTOR_SIZE) > 0) {
			...
			if((numbytes - sec_rest - (r_sec * SECTOR_SIZE)) > 0) {
				...
				block_read(ctx, tmp, 1, (seq_nr + 1 + r_sec));
				memcpy(buf + sec_rest + (r_sec * SECTOR_SIZE), tmp, r_byte);
				return numbytes;
			}
			return numbytes;
		}
		...
		block_read(ctx, tmp, 1, (seq_nr + 1));
		memcpy(buf + sec_rest, tmp, r_byte);
		return numbytes;
	}
...
s64 device_write(ps3_context *ctx, u8 *buf, u64 numbytes, s64 dev_off)
{
  ...
  if(numbytes <= sec_rest) {
		memcpy(tmp + byte_rest, buf, numbytes);
		block_write(ctx, tmp, 1, seq_nr);
		return numbytes;
	}
  
	if(numbytes > sec_rest) {
		memcpy(tmp + byte_rest, buf, sec_rest);
		block_write(ctx, tmp, 1, seq_nr);
		...
		if((remain / SECTOR_SIZE) > 0) {
			...
			if((numbytes - sec_rest - (r_sec * SECTOR_SIZE)) > 0) {
				r_byte = numbytes - sec_rest - (r_sec * SECTOR_SIZE);
				block_read(ctx, tmp, 1, (seq_nr + 1 + r_sec));
				memcpy(tmp, buf + sec_rest + (r_sec * SECTOR_SIZE), r_byte);
				block_write(ctx, tmp, 1, (seq_nr + 1 + r_sec));
				return numbytes;
			}
			return numbytes;
		}
		...
		r_byte = numbytes - sec_rest;
		block_read(ctx, tmp, 1, (seq_nr + 1));
		memcpy(tmp, buf + sec_rest, r_byte);
		block_write(ctx, tmp, 1, (seq_nr + 1));
		return numbytes;
	}
