...
u64 fat_how_many_free_bytes(ps3_context *ctx, u64 storage, struct fat_bs *fat, struct fat32_bs *fat32)
{
	...
	switch(t) {
		case 12:
			data_clu = ((fat->bs_tsec + fat->bs_nrsec) * fat->bs_ssize - data_off(fat)) / clu_size(fat);
			buf = malloc(0x02);
			fat_buf = malloc(fat_size(fat));
			...
			for(i = 2; i < data_clu; i ++) {
				...
				if((i % 2) == 0) {add->entry = add->entry & 0x00000FFF;}
		...
		break;
		case 32:
			data_clu = (fat32->bs_nrsec * fat32->bs_ssize - data_off32(fat32)) / clu_size32(fat32);
			buf = malloc(0x04);
			fat_buf = malloc(fat_size32(fat32));
